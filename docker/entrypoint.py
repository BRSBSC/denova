...
def password_hash(password):
    ...
    if not encoded or len(encoded) > 72:
        raise ValueError("DENOVA_ADMIN_PASSWORD must contain 1 to 72 UTF-8 bytes")
...
def initialize_config(target):
    if target.exists():
        ...
        return
    username = os.environ.get("DENOVA_ADMIN_USERNAME", "").strip()
    password = os.environ.get("DENOVA_ADMIN_PASSWORD", "")
    if not username or not password:
        raise ValueError("Set DENOVA_ADMIN_USERNAME and DENOVA_ADMIN_PASSWORD for the first start")
...
def main():
    ...
    os.environ.pop("DENOVA_ADMIN_USERNAME", None)
    os.environ.pop("DENOVA_ADMIN_PASSWORD", None)
